...
// denominationService =
// N4JUtil.getInstance().numistaServices.denominationService;
// }
...
// -> parseDenomination(page));

// private Mono<NumistaPage> parseDenomination(NumistaPage numistaPage) {
// Map<String, String> denominationAttr =
// NumistaParseUtils.getAttributeWithTextSingleOption(numistaPage.page,
// "#denomination",
// "value");

// if (denominationAttr == null) {
// log.debug("Can't find Denomination while parsing page with nid: {}",
// numistaPage.nid);
...
// private Mono<Denomination> handleDenominationNotFound(DatabaseException
// error, String denominationNid,
...
// private Mono<NumistaPage> linkDenominationToNType(Denomination denomination,
// NumistaPage numistaPage) {
// numistaPage.setDenomination(denomination);
// numistaPage.parsingStatus = ParsingStatus.CHANGED;

// return getNTypeService()
// .setDenomination(numistaPage.nType, denomination,
// numistaPage.colligendisUser)
// .doOnNext(result -> result.simpleFold(log))
// .thenReturn(numistaPage);
// }

// public static final String DENOMINATIONS_BY_CURRENCY_PREFIX =
// "https://en.numista.com/catalogue/get_denominations.php?";

// private Mono<Boolean> loadAndParseDenomination(Currency currency,
...
// .switchIfEmpty(Mono.error(new RuntimeException("Can't load PHP Denominations
// from URL: " + url)))
// .flatMapMany(this::extractOptionElements)
// .index()
// .flatMap(tuple -> processDenominationOption(tuple.getT1() + 1, tuple.getT2(),
// currency, user))
// .then(Mono.fromRunnable(() -> {
// if (log.isDebugEnabled()) {
// log.debug("Successfully processed all denominations for currency {}",
// currency.getNid());
// }
// }))
// .thenReturn(true)
...
// private Mono<Denomination> processDenominationOption(long index, Element
// option, Currency currency,
// ColligendisUser user) {
// String nid = option.attr("value");
// String fullName = option.text();
// String denName = fullName.contains("(") ? fullName.substring(0,
// fullName.lastIndexOf('(') - 1)
// : fullName;

// if (log.isDebugEnabled()) {
// log.debug("[{}/?] Processing Currency nid={} fullName={}", index, nid,
// fullName);
// }
...
// return Mono
// .error(new IllegalStateException("Can't find or save denomination by nid " +
// nid));
// },
// Mono::just))
// .flatMap(denomination -> getDenominationService().setCurrency(denomination,
// currency, user)
...
// return Flux.error(new IllegalStateException("OPTGROUP found in denominations
// page"));
// }

// Elements options = doc.select("option");
// if (options.isEmpty()) {
// return Flux.error(new IllegalStateException("No <option> tags found in
// currencies page"));
// }
